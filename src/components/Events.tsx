  ...
  const events: Event[] = [
    {
      ...
      detailedDescription: "A 48-hour hackathon where participants solve real-world problems using technology. Focus areas include AI/ML, Web3, IoT, and Sustainable Tech.",
      participants: 200,
    ...
    },
    {
      ...
      detailedDescription: "Learn PCB design, circuit simulation, and hands-on soldering techniques. Perfect for beginners and intermediate electronics enthusiasts.",
      participants: 45,
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-10 sm:bottom-20 right-1/4 w-40 h-40 sm:w-64 sm:h-64 rounded-full bg-neon/10 blur-3xl animate-pulse-glow" style={{ animationDelay: "1s" }} />
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-12 relative z-10">
        ...
        <div className="text-center mb-8 sm:mb-10 md:mb-12">
          <div className="flex items-center justify-center gap-2 mb-2 sm:mb-3 text-electric">
            ...
            <span className="text-xs sm:text-sm md:text-base font-semibold uppercase tracking-wider">Our Events</span>
          </div>
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-tech font-bold mb-3 sm:mb-4 md:mb-6">
            <span className="text-electric">UPCOMING</span> <span className="text-cyber">EVENTS</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8 max-w-sm sm:max-w-2xl md:max-w-4xl lg:max-w-6xl mx-auto">
          {events.map((event) => (
            ...
            >
              {/* Event Card */}
              <div className={`h-full bg-card/50 backdrop-blur-sm border border-${event.color}/20 rounded-lg p-4 sm:p-5 md:p-6 transition-all duration-300 hover:border-${event.color}/40 hover:shadow-lg hover:shadow-${event.color}/10 cursor-pointer`}>
                {/* Event Header */}
                <div className="flex items-start justify-between mb-3 sm:mb-4">
                  <div className={`text-${event.color} p-2 sm:p-3 rounded-lg bg-${event.color}/10`}>
                    {event.icon}
                  </div>
                ...
                {/* Event Title */}
                <h3 className="text-base sm:text-lg md:text-xl font-bold mb-1 sm:mb-2 text-foreground">{event.title}</h3>
                ...
                {hoveredEvent === event.id && (
                  <div className="absolute inset-0 bg-card/95 backdrop-blur-sm rounded-lg p-3 sm:p-4 md:p-6 border border-electric/30 shadow-2xl z-20 animate-fade-in">
                    <h4 className="text-sm sm:text-base md:text-lg font-bold mb-1 sm:mb-2 text-electric">What to Expect:</h4>
                    <p className="text-xs sm:text-sm md:text-base text-muted-foreground mb-2 sm:mb-3">{event.detailedDescription}</p>
                    
                    <div className="space-y-1 sm:space-y-2 mb-2 sm:mb-3">
                      <h5 className="text-xs sm:text-sm font-semibold text-cyber">Key Learnings:</h5>
